...
const modelIds = String(env.MODEL_IDS || 'gpt-image-2,gpt-image-2.5-flare,gpt-image-2.5-sunburst').split(',').map(x=>x.trim()).filter(Boolean);
...
const server=http.createServer((req,res)=>{const id=requestId(req); res.setHeader('x-relay-request-id',id); if(req.url==='/health')return json(res,200,{ok:true,service:'openai-image-relay'},id); if(req.url==='/')return json(res,200,{service:'openai-image-relay',status:'running'},id); if(!authorized(req))return json(res,401,{error:{message:'Unauthorized'}},id); if(['POST'].includes(req.method)&&/^\/v1\/images\/(edits|generations|variations)(\?|$)/.test(req.url))return proxy(req,res,id); if(req.method==='GET'&&req.url==='/v1/models')return json(res,200,{object:'list',data:modelIds.map(id=>({id,object:'model',owned_by:'relay'}))},id); json(res,404,{error:{message:'Not found'}},id);});
